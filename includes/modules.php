 ...
 *
 * The name and the one-line description shown in the settings screen live
 * apart, in px_shop_core_module_labels(): the registry is read on
 * plugins_loaded, and a translated string there would load the text domain
 * before init (WordPress 6.7 flags that as _load_textdomain_just_in_time).
 * A module a site plugin adds through the filter may still carry its own
 * 'title' / 'desc' - the settings screen falls back to them - but should
 * rather hook px_shop_core_module_labels.
 *
 * @package PxShopCore
 */
...
/**
 * Names and descriptions of the modules for the settings screen.
 *
 * Only ever called from admin, after init - see the note at the top of
 * this file for why they are not part of px_shop_core_modules().
 *
 * @return array Module id => array( 'title' => string, 'desc' => string ).
 */
function px_shop_core_module_labels() {
	static $cache = null;
	...
	/**
	 * Filters the module labels shown in the settings screen.
	 *
	 * A site plugin that adds a module through px_shop_core_modules adds
	 * its name and description here.
	 *
	 * @param array $labels Module id => array( 'title', 'desc' ).
	 */
	$cache = apply_filters( 'px_shop_core_module_labels', $labels );
